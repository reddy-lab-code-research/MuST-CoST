...
def group_list(a, precondition_topk, bsize, cls_bsize, max_tok_num=20):
...
#         print("batch.shape", batch.shape)
        ...
        if len(batch_flat[0].split()) > max_tok_num:
            for j, seq in enumerate(batch_flat):
                lines = seq.split("\n")
                new_seq = " ".join(lines[-4:])
                batches[i][j] = new_seq
        new_batches.append(batches[i].reshape((bsize, -1)))
    a_trunc = np.concatenate(new_batches, axis=1)
    return batches, a_trunc

def get_cls_results(seqs_batches, seqs_trunc, cls_model, bsize, isprint=False):
    cls_preds_list, cls_probs_list = [],[]
    
    for seqs_batch in seqs_batches:
        cls_preds_batch, cls_probs_batch, cls_logits_batch = cls_model.classify(seqs_batch)
        cls_preds_batch = cls_preds_batch.reshape((bsize, -1))
        cls_probs_batch = cls_probs_batch.reshape((bsize, -1, 2))
        cls_preds_list.append(cls_preds_batch)
        cls_probs_list.append(cls_probs_batch)
#   cls_preds: bs*precondition_topk
    cls_preds = np.concatenate(cls_preds_list, axis=1)
#     print("cls_preds.shape", cls_preds.shape)
    cls_probs = np.concatenate(cls_probs_list, axis=1)
#     print("cls_probs.shape", cls_probs.shape)
    pos_probs = cls_probs.reshape((bsize, -1, 2))[:, :, 0]
    for i, bth in enumerate(pos_probs):
        if np.amax(bth) < 0.5:
            bth.fill(1)
            pos_probs[i] = bth
#     print("pos_probs", pos_probs)
#     print("seqs_trunc.shape", seqs_trunc.shape)
    if isprint:
        for preds_batch, probs_batch, seqs_batch in zip(cls_preds, cls_probs, seqs_trunc):
            print("------------------")
            for pred, prob, seq in zip(preds_batch, probs_batch[:, 0], seqs_batch):
                print(pred, prob, seq)
    return cls_preds, cls_probs, pos_probs
...
class TransformerModel(nn.Module):
    ...
    def generate_compile(
        self, cont, cls_model, detok, dico, detokenizer, precondition_topk, condition_lambda,
        src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None
    ):
    ...
    def generate_cont(
        self, cont, cls_model, detok, dico, detokenizer, precondition_topk, condition_lambda, max_tok_num,
        src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None
    ):
        ...
        while cur_len < global_max_len:
            ...
            tensor = tensor.data[-1, :, :].type_as(src_enc)  # (bs, dim)
            scores = self.pred_layer.get_scores(tensor)  # (bs, n_words)
            if cont and cur_len > 5:
                with torch.no_grad():
                    top_logits, top_indices = scores.topk(precondition_topk, dim=1)
                    top_probs_np = F.softmax(top_logits, dim=-1).cpu().numpy()
                    gen_new = generated[:cur_len, unfinished_mask].detach().clone()
        #           gen_new:  bs * cur_len
                    gen_new = torch.transpose(gen_new, 0, 1)
        #             tplus1_candidates: cur_len * precondition_topk * bsize
                    a = gen_new.unsqueeze(1).expand(-1, precondition_topk, -1)
                    b = top_indices.unsqueeze(2)
                    tplus1_candidates = torch.cat([a, b], dim=2)[:, :, 1:] # batch x topk x seq+1, with pad dropped
                    bsize = tplus1_candidates.shape[0]
    #                 seqs: bsize * precondition_topk
                    seqs, indentifier_critic_scores = detok(tplus1_candidates, dico, detokenizer)
                    cls_bsize = 10
                    seqs_batches, seqs_trunc = group_list(seqs, precondition_topk, 
                                                          bsize, cls_bsize, 
                                                          max_tok_num=max_tok_num)
                    cls_preds, cls_probs, compilation_critic_scores = get_cls_results(seqs_batches, 
                                                                      seqs_trunc, 
                                                                      cls_model, bsize, isprint=False)
                    final_scores = compilation_critic_scores * indentifier_critic_scores
#                     with np.printoptions(precision=3, suppress=True):
#                         for seq_batch, ind_batch, comp_batch, final_batch in zip(seqs, 
#                                                                                 indentifier_critic_scores, 
#                                                                                 compilation_critic_scores,
#                                                                                 final_scores):
#                             print("------------------")
#                             flag = True
#                             for seq, ind_score, comp_score, final_score in zip(seq_batch, 
#                                                         ind_batch, 
#                                                         comp_batch, 
#                                                         final_batch):
#                                 if flag:
#                                     print(seq)
#                                     flag = False
#                                 seq_trunc = " ".join(seq.split('\n')[-1:])
#                                 print(seq_trunc, ind_score, comp_score, final_score)
                                
#                         print("top_probs_np", top_probs_np)
#                         print("condition_lambda * pos_probs_tensor", condition_lambda * pos_probs)
#                         print("full_probs", top_probs_np + condition_lambda * pos_probs)
...
#                 pos_probs_tensor = torch.tensor(pos_probs).to(scores.device)
                pos_probs_tensor = torch.tensor(indentifier_critic_scores).to(scores.device)
                full_probs = top_probs + condition_lambda * pos_probs_tensor
                
            
    #             condition_logits = cls_logits.view(bs, precondition_topk, -1)[:, :, -1] # batch x topk of last formality pred
    #             condition_logits = condition_logits - torch.log(1 + torch.exp(condition_logits))
    #             full_logits = top_logits + condition_lambda * condition_logits

                # select next words: sample or greedy
                if sample_temperature is None:
    #                 next_words = top_indices[torch.arange(bs).to(top_indices.device), torch.argmax(full_logits, dim=-1)]
                    next_words = top_indices[torch.arange(bsize).to(top_indices.device), torch.argmax(full_probs, dim=-1)]
    ...
    def generate_beam_cont(
        self,
        cont, cls_model, detok, dico, detokenizer, precondition_topk, condition_lambda,
        src_enc,
        src_len,
        tgt_lang_id,
        beam_size,
