...
def get_parser():
    ...
    parser.add_argument(
        "--conditional_generation", type=bool_flag, default=False, help="conditional generation"
    )
#     params.precont_topk, params.cont_lambda
    parser.add_argument(
        "--precont_topk", type=int, default=10, help="precont_topk"
    )
    parser.add_argument(
        "--cont_lambda", type=float, default=0.5, help="cont_lambda"
    )
    parser.add_argument(
        "--max_tok_num", type=int, default=20, help="max_tok_num"
    )
    parser.add_argument(
...
def main(params):
    ...
    # evaluation
    if params.eval_only:
        if params.conditional_generation:
            scores = evaluator.run_all_evals_cont(trainer, params.precont_topk, params.cont_lambda, params.max_tok_num)
        else:
